"""Fourier GR2 SONIC motion-tracking env with a fixed-head 27-DoF layout.

    encoder-input (600 = (2*27 + 6) * 10):  multi-future command (jpos+jvel, sonic's
        legacy [all q, all dq]->reshape(F,2n) packing) ++ anchor 6D, F=10.
    proprio       (870 = (3 + 27 + 27 + 27 + 3) * 10):  base gyro ++ joint_pos_rel ++
        dof_vel ++ last_actions ++ gravity_dir, per-term history oldest-first, H=10.
    actor total   (1470 = 600 + 870). ``SonicGR2ActorModel`` splits it internally.

Training is from scratch with one self-consistent MuJoCo joint order. The critic uses
the privileged_mf_hist path (1545 for n=27, nb=14). Action scale is
``0.25 * effort_limit / kp`` in the GR2 MJCF actuator order.
"""
...
from unilab.envs.locomotion.g1.base import Asset, ControlConfig, Sensor
...
# GR2 residual-action scale in scalar qpos / actuator order.
_SONIC_ACTION_SCALE_MUJOCO = np.array(
    [
        0.25 * 74.45 / 200.0,
        0.25 * 74.45 / 300.0, 0.25 * 74.45 / 300.0, 0.25 * 42.75 / 100.0,
        0.25 * 42.75 / 100.0, 0.25 * 17.325 / 50.0, 0.25 * 17.325 / 50.0,
        0.25 * 17.325 / 50.0,
        0.25 * 74.45 / 300.0, 0.25 * 74.45 / 300.0, 0.25 * 42.75 / 100.0,
        0.25 * 42.75 / 100.0, 0.25 * 17.325 / 50.0, 0.25 * 17.325 / 50.0,
        0.25 * 17.325 / 50.0,
        0.25 * 224.4 / 90.0, 0.25 * 95.472 / 180.0, 0.25 * 43.83 / 120.0,
        0.25 * 224.4 / 90.0, 0.25 * 43.83 / 30.0, 0.25 * 29.835 / 60.0,
        0.25 * 224.4 / 90.0, 0.25 * 95.472 / 180.0, 0.25 * 43.83 / 120.0,
        0.25 * 224.4 / 90.0, 0.25 * 43.83 / 30.0, 0.25 * 29.835 / 60.0,
    ],
    dtype=np.float32,
...
_CRITIC_BODY_NAMES: tuple[str, ...] = (
    "base",
    "left_thigh_roll_link", "left_shank_pitch_link", "left_foot_roll_link",
    "right_thigh_roll_link", "right_shank_pitch_link", "right_foot_roll_link",
    "waist_yaw_link",
    "left_upper_arm_roll_link", "left_lower_arm_pitch_link", "left_hand_yaw_link",
    "right_upper_arm_roll_link", "right_lower_arm_pitch_link", "right_hand_yaw_link",
)


class GR2Asset(Asset):
    base_name = "base"
    ...
    scales: dict[str, float] = field(
        default_factory=lambda: {
            **RewardConfig().scales,
            ...
            # Foot-slip / foot-yaw penalties. GR2's single foot-mesh contact has no
            ...
            # model), so train and deploy use the identical MJCF. Default OFF;
            ...
            # initialized conservatively for GR2. All values are per-step
            ...
            #   joint_torque_l2   : L2 of PD torque. Tune via the reward log
            #                       (target roughly -0.05 … -0.15).
            #   anti_shake_ang_vel: deadzone jitter penalty on wrist body ang-vel.
            "joint_acc_l2": -2.5e-7,
...
@registry.envcfg("GR2SonicMotionTracking")
@dataclass
class GR2SonicMotionTrackingCfg(MotionTrackingCfg):
    """SONIC GR2 tracking with fixed head and 27 policy-controlled joints.

    From-scratch full training: encoder + decoder both trainable, no warm-start, no
    joint permutation (single MuJoCo joint order). Critic = privileged_mf_hist (1545).
    """

    scene: SceneCfg = field(
        default_factory=lambda: SceneCfg(
            model_file=str(ASSETS_ROOT_PATH / "robots" / "gr2" / "scene_sonic_27dof.xml")
        )
    )
    motion_file: str | list[str] = str(ASSETS_ROOT_PATH / "motions" / "gr2" / "stand.npz")

    anchor_body_name: str = "base"
    body_names: tuple[str, ...] = _CRITIC_BODY_NAMES
    ...
    sensor: Sensor = field(
        default_factory=lambda: Sensor(
            local_linvel="baselink-velocity",
            gyro="baselink-gyro",
            upvector="baselink-upvector",
        )
    )
    asset: Asset = field(default_factory=GR2Asset)
    critic_include_future: bool = True
    # Enabled by the full-train config for the 1545-d privileged critic.
    critic_privileged_mf_hist: bool = False


@registry.env("GR2SonicMotionTracking", sim_backend="mujoco")
class GR2SonicMotionTrackingEnv(MotionTrackingEnv):
    """GR2 motion tracking with the SONIC multi-future + history actor layout."""

    _cfg: GR2SonicMotionTrackingCfg

    def __init__(self, cfg: GR2SonicMotionTrackingCfg, num_envs: int = 1, backend_type: str = "mujoco"):
        ...
        self._strict_foot_indices = np.asarray(
            [cfg.body_names.index("left_foot_roll_link"),
             cfg.body_names.index("right_foot_roll_link")], dtype=np.intp,
        )
        ...
        # From-scratch training uses one self-consistent MuJoCo joint order.
        self._jperm = np.arange(n, dtype=np.intp)
        self._action_out_perm = None

        self._enc_dim = (_CMD_PER_FRAME * n + _ANCHOR_ORI6) * self._F   # 600
        self._proprio_frame_dim = 3 + n + n + n + 3                     # 87
        self._proprio_dim = self._proprio_frame_dim * self._H          # 870
        self._sonic_actor_dim = self._enc_dim + self._proprio_dim      # 1470
        ...
        # Joint order is the MJCF actuator order, so actuator
    ...
    def _build_sonic_critic(
        ...
        """Official privileged_mf_hist value observation (1545 for n=27)."""
        dtype = get_global_dtype()
        R = dof_pos.shape[0]
        ai = self.anchor_body_idx
        env_ids = info.get("env_ids")
    ...
    def _reward_joint_torque_l2(self, ctx: Any) -> np.ndarray:
        ...
        principled smoothness term for GR2. Stateless (no reset seeding needed).
        """
    ...
    def _reward_feet_yaw(self, ctx: Any) -> np.ndarray:
        ...
        the ``motion_body_ang_vel`` failure mode where GR2's single-mesh foot, having
